...
app.get("/pharmach_list", (req, res) => {
    ...
    try {
      response = await axios.get(
        "http://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyListInfoInqire",
        {
          params: {
            serviceKey:
              "ID43EM/ixQiOJbYb9kyZ3coo6DpQnKI7F8BT7LuhMdxB6s/NyzBVFAOlvyODT/xf4aFJzFb1hYcGfEP8JrDi9A==",
            Q0: req.query.Q0, //index.html (클라이언트)에서 넘겨준 데이터는 req에 있기 때문에 req를 통해 받는다.
            Q1: req.query.Q1,
            QT: req.query.QT,
            QN: req.query.QN,
            ORD: req.query.ORD,
  ...
  api().then((response) => {
    ...
    res.json(response.data.response.body); //response에 있는 결과물(.data.response.body 즉, 약국데이터)를 그대로 /pharmach_list경로로 접속한 컴퓨터에게 전달
  });
});
